...
---
Credits: 
* https://rpubs.com/CFernandez/686167
...
# df_obj <- as(df[,2],"transactions")
# df_obj@itemInfo$labels <- df_obj@itemInfo$levels

summary(df_obj)
```

```{r}
itemFrequencyPlot(df_obj,topN=20,type="absolute", main="Absolute Item Frequency Plot")
```
```{r}
itemFrequencyPlot(df_obj,topN=20,type="relative", main="Absolute Item Frequency Plot")
```
```{r}
